...
---
性能优化, 拿到这个任务的之后经常会觉得有劲儿没处使, 正好赶上疫情,时间比较充足,花了很多时间和精力去研究如何优化,需要学习的东西很多, 逐步积累, 做个笔记:pencil::star2:
...
+ **2/8原则**
    ...
    程序的性能往往也遵循28原则,也就是说80%的性能开销来自20%的代码. 通过Perf火焰图我们可以很快的定位到这20%的代码在哪里, 也就是快速定位性能瓶颈, 并对其进行优化.讲到这里眼泪也就出来了,这20%的代码往往都是公司的核心呀,公司的核心那不是你想优化就能优化的,不是说咱实力不行,而是这代码大概率是没有权限看的.如果在28原则里你也遇到了这样的问题,那也不要慌,毕竟还剩下80%的代码,挤一挤也还是能挤出来点的:joy:.
...
+ **代码层面的性能优化**
    ...
    + 善用std::move避免不必要的拷贝.
    + 减少循环的层数、减少递归、在循环中少声明变量、尽量把循环体内的表达式抽到循环外、条件表达的中的多个条件判断的次序.
...
+ **算法**
    ...
    算法优化非常的重要, 但是这块需要付出的时间和精力相对较多, 但是一旦有所突破,效果也会十分惊人.
...
+ **补充**
    ...
    代码最终是交到cpu手里执行的, 所以高性能代码一定是建立在对cpu充分了解的基础上的, 详细可以见陈浩哥的这篇文章 [与程序员相关的CPU缓存知识](https://coolshell.cn/articles/20793.html)

    - **调整线程数**

    多线程的程序在高峰期性能表现出现下降时, 如果 **机器的负载不高**, 一定要看 **各任务的线程** 是否已经跑满单核(htop, pstack等), 如果已经跑满, 增加该任务的线程数会有显著的效果. 
