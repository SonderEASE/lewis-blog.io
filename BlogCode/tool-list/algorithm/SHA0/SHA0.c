...
static int SHA_Update(SHA_CTX *c, const void *data_, size_t len){
    ...
    l=(c->Nl+(((HASH_LONG)len)<<3))&0xffffffffUL;//乘以8（1个char型==8 bits）
    if (l < c->Nl)
        c->Nh++;
...
static int SHA_Final(unsigned char *md, HASH_CTX *c){
