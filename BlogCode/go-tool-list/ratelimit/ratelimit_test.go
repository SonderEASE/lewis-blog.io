...
// 自增测试用例
func (rateLimitSuite) TestTakeAvailableWitRate(c *gc.C) {
	rateLimit := QpsLimit{
		Capacity: 50,
	}
	count := 0
	//rateLimit.bucket = NewBucketWithQuantum(1 * time.Millisecond, 50, 1)
	rateLimit.Bucket = NewBucketWithRate(1000, 50)
	for {
		if rateLimit.Bucket.TakeAvailable(1) == 0 {
			logger.Errorf("[TestTakeAvailableWitRate] Failed")
			return
		}
		count++
		if count > 100 {
	...
	logger.Info("[TestTakeAvailableWitRate] Passed")
}
