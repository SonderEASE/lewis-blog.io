...
func (tb *Bucket) ImproveQps(percent float64, rType int) {
	a := tb.Available()
	tb.mu.Lock()
	defer tb.mu.Unlock()
	interval := float64(tb.fillInterval) * float64(1-percent)
	tb.fillInterval = time.Duration(interval)
	tb.setavailableTokens(tb.currentTick(tb.clock.Now()), a)
	logger.Infof("[QPS] %d rate improved to %.2f", rType, tb.Rate())
}

func (tb *Bucket) ReduceQps(percent float64, rType int) {
	a := tb.Available()
	tb.mu.Lock()
	defer tb.mu.Unlock()
	interval := float64(tb.fillInterval) * float64(1+percent)
	tb.fillInterval = time.Duration(interval)
	tb.setavailableTokens(tb.currentTick(tb.clock.Now()), a)
	logger.Infof("[QPS] %d rate reduce to %.2f", rType, tb.Rate())
}
...
// fillInterval
func (tb *Bucket) FillInterval() float64 {
	return float64(tb.fillInterval)
}
...
// setavailableTokens
func (tb *Bucket) setavailableTokens(tick int64, a int64) {
	tb.latestTick = tick
	if tb.availableTokens >= tb.capacity {
		return
	}
	tb.availableTokens = a
	if tb.availableTokens > tb.capacity {
